...
const AllBoxNewSong = () => {
  ...
  const { list: listNewSong, loading, err } = useSelector(
    (state: AppState) => state.listSong
  );
  ...
  const onChangeMusic = (pageOnChange: number) => {
    setPage(pageOnChange);
  };

  return (
    <div className="all-box-new-song-wrapper">
      <BoxTitleNewMusic
      ...
      : //    : err == true || listNewSong.length === 0
        ...
        <BoxListNewSong err={err} start={start} end={end} arr={listNewSong} />
      }
    </div>
  );
};
